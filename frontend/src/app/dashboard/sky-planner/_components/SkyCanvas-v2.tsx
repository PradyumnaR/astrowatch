...
function getCloudShapes(cloudCover: number): CloudShape[] {
  const baseOpacity = Math.max(cloudCover / 100, 0.25); // ← was 0.55

  console.log(cloudCover, baseOpacity);

  if (cloudCover < 10) return [];

  if (cloudCover < 30)
    return [{ cx: 600, cy: 60, rx: 50, ry: 14, opacity: baseOpacity }];
...
function MoonPhase({
  ...
  const isWaxing = phase.toLowerCase().includes("waxing") || isFull;
  const isWaning =
    phase.toLowerCase().includes("waning") || phase === "Last quarter";

  if (isNew) return null;

  const glowOpacity = 0.08 + illumination * 0.15;

  // how far the dark overlay shifts
  // illumination 1.0 = full → offset pushes dark circle off screen
  // illumination 0.5 = quarter → dark circle covers half
  // illumination 0.2 = crescent → dark circle covers most
  const offset = r - illumination * r * 2;

  const moonId = `moon-clip-${phase.replace(/\s+/g, "-")}`;
  ...
  return (
    <g>
      ...
      {/* ambient glow */}
      <circle cx={cx} cy={cy} r={r + 6} fill="white" opacity={glowOpacity} />

      {/* dark base */}
      <circle cx={cx} cy={cy} r={r} fill="#1a1a3a" opacity=".9" />

      {/* lit portion clipped to moon circle */}
      <g clipPath={`url(#${moonId})`}>
        {/* lit surface */}
        <circle cx={cx} cy={cy} r={r} fill="#e8e0c8" opacity=".95" />
        {/* dark overlay — direction flips for waxing vs waning */}
        {!isFull && (
          <circle
            cx={cx + (isWaning ? -offset : offset)}
            cy={cy}
            r={r}
            fill="#1a1a3a"
            opacity=".95"
...
function CloudLayer({ cloudCover }: { cloudCover: number }) {
  if (cloudCover < 10) return null;

  const clouds = getCloudShapes(cloudCover);
    ...
    <g>
      {clouds.map((cloud, i) => (
        <g key={i} opacity={cloud.opacity}>
          ...
          {/* Left Top Puff */}
          <ellipse
            cx={cloud.cx - cloud.rx * 0.3}
            cy={cloud.cy - cloud.ry * 0.45}
            rx={cloud.rx * 0.55}
            ry={cloud.ry * 0.75}
            fill="rgba(180,190,210,1)"
          />
          {/* Center Top Puff (Higher) */}
          <ellipse
            cx={cloud.cx + cloud.rx * 0.05}
            cy={cloud.cy - cloud.ry * 0.65}
            rx={cloud.rx * 0.5}
            ry={cloud.ry * 0.95}
            fill="rgba(180,190,210,1)"
          />
          {/* Right Top Puff */}
          <ellipse
            cx={cloud.cx + cloud.rx * 0.4}
            cy={cloud.cy - cloud.ry * 0.3}
            rx={cloud.rx * 0.45}
            ry={cloud.ry * 0.65}
            fill="rgba(180,190,210,1)"
          />
...
export default function SkyCanvas() {
  ...
  const arc = useMemo(() => {
    ...
    const color = satColor(selectedPass.satid);
    const offsetPath = `path('M ${x1} ${HORIZON_Y} Q ${cx} ${cy} ${x2} ${HORIZON_Y}')`;

    return {
      path: `M ${x1} ${HORIZON_Y} Q ${cx} ${cy} ${x2} ${HORIZON_Y}`,
      startX: x1,
      peakX: cx,
      peakY: cy,
      endX: x2,
  ...
  // weather from selected pass
  const cloudCover = selectedPass?.cloudCover ?? 0;
  const moonPhase = selectedPass?.moonPhase ?? "New moon";
  const moonIllumination = selectedPass?.moonIllumination ?? 0;

  // star brightness reduces as clouds increase
  const starOpacity = Math.max(0.15, 1 - cloudCover / 100);
  ...
  return (
    ...
    >
      <style>{`
        @keyframes tw-a {
          0%,100% { opacity: ${(starOpacity * 0.9).toFixed(2)}; }
          50%     { opacity: ${(starOpacity * 0.3).toFixed(2)}; }
        }
        @keyframes tw-b {
          0%,100% { opacity: ${(starOpacity * 0.6).toFixed(2)}; }
          50%     { opacity: ${(starOpacity * 0.15).toFixed(2)}; }
        }
        @keyframes tw-c {
          0%,100% { opacity: ${(starOpacity * 0.5).toFixed(2)}; }
          33%     { opacity: ${(starOpacity * 0.95).toFixed(2)}; }
          66%     { opacity: ${(starOpacity * 0.2).toFixed(2)}; }
        }
        @keyframes ring {
          0%   { r: 5;  opacity: .7; }
          100% { r: 18; opacity: 0;  }
      ...
      >
        ...
        {/* stars — opacity driven by cloud cover */}
        ...
        <MoonPhase phase={moonPhase} illumination={moonIllumination} />

        {/* cloud layer — over stars, under arc */}
        ...
        {/* bottom left — cloud cover indicator */}
        {cloudCover > 0 && (
          <text
            x={W - 250}
            y="18"
